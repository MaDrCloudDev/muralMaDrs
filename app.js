...
const dbUrl =
	process.env.DB_URL || 'mongodb://localhost:27017/mural-madrs';
...
const store = MongoStore.create({
	mongoUrl: dbUrl,
	touchAfter: 24 * 3600,
	crypto: false, // Disable encryption to avoid decryption errors
});
...
app.use((req, res, next) => {
	console.log(
		'Flash middleware initialized, req.flash:',
		typeof req.flash
	);
	next();
});
...
app.use((req, res, next) => {
	try {
		console.log(
			'Middleware: Setting res.locals.currentUser',
			req.user || null
		);
		console.log(
			'Flash success:',
